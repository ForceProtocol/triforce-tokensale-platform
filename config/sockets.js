...
module.exports.sockets = {
  ...
  ***************************************************************************/
   beforeConnect: function(handshake, cb) {
		// Subscribe to triforce - when it updates
		/*
		Triforce.findOne({id:1}).exec(function (err, triforce) {

			var totalUsers = Number(triforce.onlineUsers) + 1;

			if(totalUsers < 3){
				totalUsers = 3;
			}

			Triforce.update({id:1},{onlineUsers:totalUsers}).exec(function(err,updated){
			});

			sails.sockets.blast('onlineUsers', { totalUsers: totalUsers });
